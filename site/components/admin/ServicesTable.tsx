...
export default function ServicesTable({ items }: { items: Item[] }) {
  ...
  function onPublish(id: string) {
    ...
    startTransition(async () => {
      const res = await publishService(id);
      setBusyId(null);
  ...
  function onDelete(id: string, name: string) {
    if (!confirm(`Видалити послугу «${name}»? Цю дію не можна скасувати.`)) return;
    setError(null);
    setBusyId(id);
    startTransition(async () => {
              ...
              return (
                <tr key={it.id}>
                  ...
                  <td className="px-4 py-3">
                    ...
                    {it.hidden && (
                      <span className="ml-2 rounded-full bg-slate-200 px-2 py-1 text-xs font-medium text-slate-600">
                        Прихована
                      </span>
                    )}
                  </td>
