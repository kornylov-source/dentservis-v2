...
type Item = { id: string; slug: string; name: string; photo: string; state: State };
...
export default function DoctorsTable({ items }: { items: Item[] }) {
  ...
  function onPublish(id: string) {
    ...
    startTransition(async () => {
      const res = await publishDoctor(id);
      setBusyId(null);
  ...
  function onDelete(id: string, name: string) {
    if (!confirm(`Видалити лікаря «${name}»? Цю дію не можна скасувати.`)) return;
    setError(null);
    setBusyId(id);
    startTransition(async () => {
